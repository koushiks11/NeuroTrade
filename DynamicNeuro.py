...
def predict_closing_price(input_date, selected_stock):
    ...
    if days >= 30:
        ...
        if fetched_date_high != -1:
            ...
            st.write(f"**The date corresponds to a stock market holiday or non-trading day. Choose another date**")
...
st.sidebar.write("**Note: The input date should not be more than 30 days from today. Saturdays and Sundays are not valid input dates.**")
input_date = st.sidebar.date_input("Enter a date", datetime.today())
